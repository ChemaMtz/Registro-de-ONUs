...
async function populateCatalogs() {
  ...
  snapshot.docs.forEach(doc => {
    // Ignorar el propio documento de configuración si ya existe
    if (doc.id === '---config_catalogs---') return;
  ...
  console.log('Guardando catálogos en Firestore (onus/---config_catalogs---)...');
  await db.collection('onus').doc('---config_catalogs---').set(catalogData, { merge: true });

  console.log('✅ Catálogos actualizados correctamente en la nueva ubicación.');
}
